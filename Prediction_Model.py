...
from lxt.models.llama import LlamaConfig
...
class LLM_remote:
    def __init__(self,model_id,GPU_Savings,max_memory,tokenizer,Relevance_Map_Method,Baselines,SG_iterations=10):
        ...
        if self.Relevance_Map_Method=='LXT':
            config = LlamaConfig.from_pretrained(model_id)
            config._attn_implementation = "eager"
            if self.device_map!='cuda':
                raise ValueError("LXT was only tested for device_map='cuda'.")
            self.model = LlamaForCausalLM.from_pretrained(model_id, config=config, torch_dtype=torch.bfloat16, device_map=self.device_map)
        ...
        if self.Relevance_Map_Method not in ["LXT"]:
        ...
        if self.Relevance_Map_Method=='vanilla_gradient' or self.Relevance_Map_Method=='LXT':
    ...
    def forward_pass_captum(self,Task_Text_Tokens,Result_Target):
    ...
    def forward_pass_LXT(self,Task_Text_Tokens):
        #print('!'*100)
        input_embeds = self.model.get_input_embeddings()(Task_Text_Tokens)
        #print(self.model.dtype)
        #print(input_embeds.dtype)

        # inference and get the maximum logit at the last position
        M_outputs = self.model(inputs_embeds=input_embeds.requires_grad_(), use_cache=False)
        #print(M_outputs.logits.dtype)
        return M_outputs
    ...
    def forward_pass(self,Task_Text_Tokens,Result_Target=None,Use_Captum=False):
        ...
        if self.GPU_Savings:
            with torch.autograd.graph.save_on_cpu(): #Optimized for GPU Memory savings
                if Use_Captum:
                    with autocast():
                        M_outputs = self.forward_pass_captum(self,Task_Text_Tokens,Result_Target)
                elif self.Relevance_Map_Method=='LXT':
                    M_outputs = self.forward_pass_LXT(Task_Text_Tokens)
                else:
                    with autocast():
    ...
    def backward_pass(self,loss):
        ...
        if self.Relevance_Map_Method=='LXT':
            #print(loss)
            loss.backward(loss)
        elif self.GPU_Savings or (isinstance(self.max_memory, str) and self.device_map!="cpu"):
    ...
    def tensors_to_lists(self,data): #Helper function which ensures the detachment of the gradient
        if isinstance(data, torch.Tensor):  # Check if it's a tensor
            return data.cpu().detach().to(torch.float32).numpy()
    ...
    def helper_hiden_rep(self,data): #Outdated
        if isinstance(data, torch.Tensor):  # Check if it's a tensor
    ...
    def Prepare_Input(self,Task_Text,Task_Result):
        ...
        if self.Relevance_Map_Method=='LXT':
            Task_Text_Tokens=self.tokenizer(Task_Text, return_tensors="pt", add_special_tokens=True).input_ids.to(self.model.device)
        else:
    ...
    def get_results_hidden_Representation(self,Task_Text,Task_Result):
        ...
        return self.extracted_outputs,Correctly_classified
