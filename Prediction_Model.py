...
def create_llm_remote(model_id, GPU_Savings, max_memory, tokenizer,Relevance_Map_Method,num_gpus=0,num_cpus=1):
    return ray.remote(LLM_remote).options(num_gpus=num_gpus,num_cpus=num_cpus).remote(model_id, GPU_Savings, max_memory,tokenizer,Relevance_Map_Method)