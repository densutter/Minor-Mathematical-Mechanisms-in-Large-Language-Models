...
class Relevance_Map(Prediction_Helpers.Prediction_Helper):
    ...
    def Get_Relevance_Map(self, Number_of_samples):
        ...
        while self.Metadata[self.task_2.Task_Name]['Computed']<Number_of_samples or self.Metadata[self.task_1.Task_Name]['Computed']<Number_of_samples:
            ...
            if self.verbose:
                processed_percentage=(self.Metadata[self.task_1.Task_Name]['Computed']+self.Metadata[self.task_2.Task_Name]['Computed'])/(2*Number_of_samples)
                print('[INFO] Processing:',actual_task_key,'; Processed',self.task_1.Task_Name,':',self.Metadata[self.task_1.Task_Name]['Computed'],',', self.task_2.Task_Name,':',self.Metadata[self.task_2.Task_Name]['Computed'],"Remaining:",self.TimeMeasurer.stop(processed_percentage),flush=True)
